...
$request->validate([
    ...
    'product_name' => 'required|max:255|regex:/^[A-Za-z_][A-Za-z\d_]*$/',
    'brand_id' => 'required',
   ...
   'product_short_descp' =>'required',
    ...
    'product_name.regex'=>"NOT a valid Product name"
          ...
          $save_url = 'upload/products/thambnail/'.$name_gen;
            ...
            if($request->hot_deals||$request->special_offer||$request->special_deals)
            {
                $deals = new DealsTime();
    ...
    public function ProductDelete($id){
       ...
       return redirect()->back()->with($notification);
        ...
        public function HotDeals()
        {
            ...
            return view('backend.product.product_deal',compact('products'));
        }

        public function HotDealsID($id)
        {
        ...
        public function HotDealsStore(Request $request)
        {
            if(isset($request->id))
            {
                if(isset($request->hot_deal))
                {
                    ...
                    $deal = DealsTime::where('product_id',$request->id)->first();
                    $deal->hot_deals = $request->hot_deal;
                    $deal->save();
                ...
                {
                    $deal = DealsTime::where('product_id',$request->id)->first();
                    $deal->special_offer = $request->special_offer;
                    $deal->save();
                    return redirect()->back();
                }
                ...
                {
                    $deal = DealsTime::where('product_id',$request->id)->first();
                    $deal->special_deals = $request->special_deal;
                    $deal->save();
                    return redirect()->back();
                }
            }
